import { FIELDS, STATS, RELATIONS, ROLE_ICONS, identity, profileFields, completeDraft, importCharacters, readCharacterFile, keyName, clean } from './npc-core.js?v=0.30.0';
import { portraitEditor, preparePortrait, croppedPortrait } from './npc-portraits.js?v=0.30.0';
import { element, icon, speakerHeader, narrative, createChatPresentation } from './npc-chat.js?v=0.30.0';
...
export function createNpcWorkspace(api) {
    ...
    const sheet=document.createElement('link');sheet.rel='stylesheet';sheet.href=new URL('./npc-ui.css?v=0.30.0',import.meta.url).href;document.head.append(sheet);
    ...
    function ensureDialog(){
        ...
        dialog.innerHTML=`<header class="trpg-manager-top"><div><small>CHARACTER ARCHIVE / TRETARESIA</small><h2 id="trpg-manager-title">NPC MANAGEMENT</h2></div><button type="button" data-close aria-label="ปิด">×</button></header>
            ...
            <footer class="trpg-manager-footer"><span role="status" aria-live="polite"></span><span>CHAT-BOUND ARCHIVE · v0.30.0</span></footer>`;
